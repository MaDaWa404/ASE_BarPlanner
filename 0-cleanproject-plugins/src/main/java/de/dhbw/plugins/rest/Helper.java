...
public class Helper {
    ...
    static String getIdFromRequest(HttpServletRequest request) throws MyException {
        String id = (String) request.getSession().getAttribute("person");
        if (id == null) throw new MyException(MyErrorCode.NO_ID);
        return id;
    }

    static Bar getBarFromRequest(HttpServletRequest request, PersonService personService, BarService barService) throws MyException {
